...
Below is an example `tibble` with a jumbled `name` column (`JJJSBefore`) similar to the one above. I chose to use commas and spaces because they're a little easier to read. 
...
This gets the `JJJSBefore` data into a tidy format, but it would require some additional wrangling to get the data back into the two columns the question was looking for. 

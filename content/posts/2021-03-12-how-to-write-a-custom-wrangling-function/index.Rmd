...
Have you ever had trouble finding the appropriate R function for your wrangling needs? I've found I can tackle 80-90% of data wrangling with `dplyr`, `tidyr`, and `stringr`, but occaisionally I run into a problem these functions can't quite handle. 

For example, I recently ran into a situation where I was looking for a function that could split the contents of a character column on a specified pattern, but keep the original pattern. After some searching, I found this post on [Stackoverflow question](https://stackoverflow.com/questions/4350440/split-data-frame-string-column-into-multiple-columns): 

The original question:

> **Stack Overflow: Split data frame string column into multiple columns**
...
Fortunately, this particular question was posed almost ten year before the time of this writing, so there are quite a few solutions listed. I liked the approach [Yannis-P](https://stackoverflow.com/users/1885713/yannis-p) took because 1) it offers a `tidyverse` solution *and* 2) it's written as a function. 
...
Below is the function [Yannis-P](https://stackoverflow.com/users/1885713/yannis-p) wrote in his SO solution:
...
![](images/matrix-indeces.png){width=80%}
...
## The New `split_into_multiple()` Function!

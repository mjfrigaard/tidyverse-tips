...
[File paths can be a problem in R](https://stackoverflow.com/questions/8425409/file-path-issues-in-r-using-windows-hex-digits-in-character-string-error) depending on what operating system you use. Windows uses a backslash (`\`), and everyone else uses forward slashes (`/`), which creates a lot of file paths with *two* forward slashes. As you can imagine, this can be a problem when projects get passed from one person to another, because file paths are expecting certain files to be in certain places. The `fs` package handles this problem with tidy file paths. 
